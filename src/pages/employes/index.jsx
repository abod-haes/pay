...
const Employee = () => {
  ...
  const { data: departmentData, isLoading: isLoadingData } = DepartmentsQueries.GetAll({});
  ...
  const rowData = useMemo(
    () =>
      data?.data?.map((item, index) => ({
        ...
        department: truncateText({ text: item.department.name, maxLength: 20 }) || "-",
  ...
  const departmentOptions = useMemo(
    () =>
      departmentData?.data?.map(employee => ({
        label: employee.name,
        value: employee.id,
      })) || [],
    [departmentData?.data]
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "employee", header: t("staff.employee") },
  ...
  const handleEdit = row => navigate(`/staff/employee/${row.id}`);
  ...
  const handleShow = row => navigate(`/staff/employee/${row.id}/show`);
      ...
      component: (
        <div className="w-[400px]">
          <SelectField
            ...
            placeholder={t("department.choose")}
    ...
    <>
      <div>
        <BreadCrumb
          ...
          link={"/staff/employee"}
          onClick={() => navigate("/staff/employee/add")}
          buttonText={t("staff.add-employee")}
          hideBrimaryButton={
            !hasPermissionFunction({
              group: PERMISSION_GROUP.Employee,
