...
export default function EmployeeShow() {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: "price",
        header: t("employee.reward-value"),
        enableColumnFilter: true,
      },
      {
        accessorKey: "reason",
  ...
  const columns3 = useMemo(
    () => [
      ...
      {
        accessorKey: "price",
        header: t("employee.deduction-value"),
        enableColumnFilter: true,
      },
      {
        accessorKey: "reason",
  ...
  const employeeDetails = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: oneData?.data.department?.name || "-",
      },
      {
        label: t("staff.job-title"),
        value: oneData?.data.job_title?.name || "-",
      },
      { label: t("common.address"), value: oneData?.data.address || "-" },
    ];
  }, [oneData, t]);
  const employeeDetails2 = useMemo(() => {
    if (!oneData?.data) return [];
    ...
    return [
      ...
      },
      {
        ...
        value: oneData?.data.salary?.value ? `${oneData?.data.salary.value} ` : "0",
      },
      {
        label: t("staff.hire-date"),
        value: oneData?.data.appointment_date
          ? new Date(oneData?.data.appointment_date).toLocaleDateString()
          : "-",
      },
      {
        label: t("staff.start-date"),
        value: oneData?.data.start_date
          ? new Date(oneData?.data.start_date).toLocaleDateString()
          : "-",
      },
      { label: t("common.user"), value: oneData?.data.username || "-" },
      {
        label: t("permissions.permission"),
        value: oneData?.data.role?.name || "-",
      },
    ];
  }, [oneData, t]);
