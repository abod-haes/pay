...
export default function BookingDetails() {
  ...
  const { isLoading, data, refetch } = useInjectionsQueries.GetOne({ id });
    ...
    const menuItems = [
      {
        ...
        onClick: () => navigate(`/injections/add-injections/${encryptId(dataToReset.id)}`),
        show: status !== "done" && status !== "delayed" && status !== "cancel",
      },
      {
        label: t("hair.cancel"),
  ...
  return (
    <div className="relative">
      ...
      <Card otherStyle={"mb-8 !py-6"}>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 w-full md:w-[80%]">
