...
import { apis } from "@/apis/booking/injections/api";
...
const AddEmployees = () => {
  ...
  const { isLoading, data, refetch, isRefetching } = useInjectionsQueries.GetOne({ id });
  ...
  const p = items?.find(p => p.value === watch("patient_id")?.value);
  ...
  useEffect(() => {
    if (p && dirtyFields?.patient_id) {
      ...
      setValue("birth_date", p.birth_date?.split("T")[0]);
  ...
  useEffect(() => {
    const patientId = watch("patient_id")?.value;
  ...
  }, [data?.data, isEdit, isAdd, isRefetching, isLoading]);
