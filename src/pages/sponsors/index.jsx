...
const Sponsors = () => {
  ...
  const { data, isLoading, refetch } = useSponsorsQueries.GetAll({
  ...
  const rowData = useMemo(
    () =>
      data?.data?.map((item, index) => ({
        ...
        full_name: truncateText({ text: item.full_name, maxLength: 20 }),
        address: truncateText({ text: item.address, maxLength: 30 }) || "-",
        notes: truncateText({ text: item.notes, maxLength: 30 }) || "-",
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "full_name", header: t("delayed.fullName") },
      {
        accessorKey: "phone",
  ...
  const handleEdit = row => navigate(`/sponsors/${row.id}`);
    ...
    const menuItems = [
      {
        ...
        show: hasPermissionFunction({
          group: PERMISSION_GROUP.SPONSOR,
    ...
    <>
      <div>
        <BreadCrumb
          ...
          buttonText={t("sponsors.add-sponsors")}
          hideBrimaryButton={
        ...
        <Table
          ...
          onEdit={handleEdit}
          permissionGroup={Permissions.SPONSOR}
          hasSearch={true}
      ...
      {state.isDeleteModalOpen && (
        <DeleteModal
          ...
          title={t("sponsors.delete-sponsors")}
          warning={t("delayed.warning")}
          deleteText={t("sponsors.delete-sponsors")}
