...
const ActionOperationBooking = () => {
    ...
    if (JSON.stringify(prevDataRef.current) !== JSON.stringify(d)) {
      reset({
        ...
        phone_number1: d.patient.first_phone_number
          ? `+${d.patient.first_phone_number_country_code}${d.patient.first_phone_number}`
          : "",
        phone_number2: d.patient.second_phone_number
          ? `+${d.patient.second_phone_number_country_code}${d.patient.second_phone_number}`
          : "",

        first_phone_number_country_code: d.patient.first_phone_number_country_code,
        second_phone_number_country_code: d.patient.second_phone_number_country_code,
      });

      setIsSelected(d?.date?.split(" ")[1]);
      setSelectedDate(d?.date?.split(" ")[0].replaceAll("/", "-"));
  ...
  const onSubmit = async data => {
    try {
      const dataToSend = {
        ...
        section: "hair_transplant",
        service_id: services?.find(item => item.type === "eyebrow_transplant")?.value,
  ...
  const TITLE = isAdd
    ? t("surgeries.add-booking") + " - " + t("surgeries.eyebrow-transplant")
    : t("booking.edit");
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="flex flex-col lg:flex-row gap-[20px]">
            <div className="space-y-6 w-full lg:w-[80%] relative">
              ...
              />
              <MedicalInformation control={control} setValue={setValue} watch={watch} />
            ...
            <SurgeriesArchaive
              ...
              title1={t("surgeries.before-eyebrow")}
              title2={t("surgeries.after-eyebrow")}
            />
          </div>
          <CustomFlexButtons
            gap="gap-4"
...
export default ActionOperationBooking;
