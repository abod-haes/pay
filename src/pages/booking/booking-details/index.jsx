...
export default function BookingDetails() {
  ...
  const bookingInfo = [
    ...
    },
    {
      ...
      value:
        dataToReset?.date?.split(" ")[1]?.slice(0, 4) +
        +" " +
  ...
  const personalInfo = [
    ...
    },
    {
      ...
      value: formatDateOrTime({ input: dataToReset?.patient?.register_date, type: "date" }),
    },
  ];
    ...
    const menuItems = [
      {
        ...
        onClick: () => navigate(`/booking/${state}/${encryptId(dataToReset.id)}`),
        show:
          status !== "done" &&
          status !== "delayed" &&
          status !== "cancel" &&
