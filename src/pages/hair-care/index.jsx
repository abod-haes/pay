...
const Booking = () => {
  ...
  const { data, isLoading, refetch } = useHarCareQueries.GetAll({
  ...
  const handleEdit = row => navigate(`/hair/add-hair-care/${encryptId(row.id)}`);

  const handleDelete = row => {
    dispatch({ type: branchesActions.openDeleteModal, payload: true });
    dispatch({ type: branchesActions.selectedId, payload: row.id });
  };
  const handleShow = row => navigate(`/hair/${encryptId(row.id)}`, { state: "hair" });
  ...
  return (
    <div>
      <BreadCrumb
        title={t("hair.name")}
        onClick={() => navigate("/hair/add-hair-care/add")}
        hideBrimaryButton={
          !hasPermissionFunction({
            group: PERMISSION_GROUP.HairCare,
            type: PERMISSION_ACTION.create,
      ...
      <Table
        ...
        onEdit={handleEdit}
        onShow={handleShow}
        permissionGroup={PERMISSION_GROUP.HairCare}
        hasSearch={true}
