          ...
          <div className="flex-grow">
            ...
            >
              ...
              <div className="flex flex-col justify-center items-center gap-2">
                ...
                <p className="font-main  text-[1.2rem]">{t("print.bill")}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              <div>
                <div className="p-4 rounded-lg" style={{ backgroundColor: "#f8fafc" }}>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <p style={{ color: "#0f172a" }}>{invoiceData?.vendor?.name || "-"}</p>
                  </div>
                  <div className="mb-2 flex items-center gap-2">
              ...
              <div>
                <div className="p-4 rounded-lg" style={{ backgroundColor: "#f8fafc" }}>
                  <div className="mb-2 flex items-center gap-2">
                    <span style={{ color: "#475569" }}>{t("print.billType") + ":"}</span>
                    <p style={{ color: "#0f172a" }}>
                      {invoiceData?.total === invoiceData?.paid_amount
                        ? t("print.type1")
                        : t("print.type2")}
                    </p>
                  </div>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <p style={{ color: "#0f172a" }}>{invoiceData?.date}</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <table className="w-full" style={{ tableLayout: "fixed" }}>
                  <thead>
                    ...
                    >
                      ...
                      <th style={{ width: "12%", padding: "12px 0px" }}>{t("print.price")}</th>
                      <th style={{ width: "17%", padding: "12px 0px" }}>{t("print.total")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {invoiceData?.details?.map((detail, index) => {
                      const qty = parseFloat(detail.quantity);
                      const sell = parseFloat(detail.selling_price);
                      const buy = parseFloat(detail.purchase_price);
                      return (
                        <tr key={detail.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                          ...
                          <td className="text-center py-2">{buy.toFixed(2)}</td>{" "}
                          <td className="text-center py-2">{(qty * buy).toFixed(2)}</td>{" "}
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div className="flex justify-between  mb-8">
              <div className="w-50">
                <div className="flex justify-between py-2">
                  ...
                  <span style={{ color: "#0f172a" }}>{invoiceData?.notes}</span>
                </div>
              </div>
              <div className="w-80">
                <div
                ...
                >
                  <span style={{ color: "#475569" }}>{t("print.totalPrice") + ":"}</span>
                ...
                <div
                  ...
                  style={{ backgroundColor: "#06b6d4", color: "#ffffff" }}
                >
                  <span>{t("print.arriving") + ":"}</span>
                  <span>{invoiceData?.paid_amount}</span>
                </div>
              </div>
            </div>
          </div>
