...
export default function AddDeduction() {
  ...
  const { data: deductionData } = useDeductionQueries.GetOne({ id });
  ...
  useEffect(() => {
    if ((isEdit || isShow) && deductionData?.data) {
      const emp = deductionData.data?.data;
  ...
  }, [isEdit, isShow, deductionData, reset]);
  ...
  const TITLE = isAdd ? t("deduction.add") : isShow ? t("deduction.show") : t("deduction.edit");

  return (
    <div>
      <BreadCrumb isAdd title={TITLE} link={"/deduction"} />
