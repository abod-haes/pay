...
import { apis } from "@/apis/deduction/api";
...
export default function Deduction() {
  ...
  const { data, isLoading, refetch } = useDeductionQueries.GetAll({
  ...
  const rowData = useMemo(
    () =>
      data?.data?.map((item, index) => ({
        ...
        deduction: item.value + " " + t("code"),
        jobTitle: truncateText({ text: item.job_title?.full_name || "", maxLength: 5 }),

        deductionDate: formatDateOrTime({ input: item.date, type: "date" }),
  ...
  const columns = useMemo(
    () => [
      ...
      { accessorKey: "deduction", header: t("deduction.value") },
      { accessorKey: "jobTitle", header: t("staff.job-title") },
      { accessorKey: "deductionDate", header: t("deduction.date") },
  ...
  const handleEdit = row => navigate(`/staff/deduction/${row.id}`);
  const handleShow = row => navigate(`/staff/deduction/${row.id}?show=true`);
  ...
  return (
    <div>
      <BreadCrumb
        ...
        onClick={() => navigate("/staff/deduction/add")}
        buttonText={t("deduction.add")}
        hideBrimaryButton={
      ...
      {state.isDeleteModalOpen && (
        <DeleteModal
          ...
          title={t("deduction.delete")}
          warning={t("item.warning")}
          deleteText={t("deduction.delete")}
